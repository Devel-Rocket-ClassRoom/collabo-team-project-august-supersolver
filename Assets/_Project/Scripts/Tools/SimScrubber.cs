...
{
    /// <summary>
    /// 디버그 뷰어. 스테이지와 솔루션을 받아
    /// 그대로 재생한다. 레벨 데이터는 들지 않고
    /// 그리기와 스텝 이동만 한다.
    /// </summary>
    public sealed class SimScrubber : MonoBehaviour
    {
        ...
        static Entry[] BuildCatalog()
        {
            ...
            return new[]
            {
                ...
                Stage("FlatRest → Stalled", TestLevels.FlatRest, () => null),
                Stage("FreeFall → Fail", TestLevels.FreeFall, () => null),
                Stage("LongRoll (계속 굴러감)", TestLevels.LongRoll, () => null),
                Stage("PivotSwing (회전축)", TestLevels.PivotSwing, TestLevels.PivotSolution),
                Stage("자유 물체 낙하", TestLevels.FlatRest, TestLevels.FreeBodySolution),

                // 시드 7 = FragBombPitStage 의 기본값.
                Stage("파편 구덩이 → Fail", TestLevels.FragBombPit, () => null, seed: 7),
                Stage("파편 멀리 → Stalled", TestLevels.FragBombFarAway, () => null, seed: 7),
            };
#else
            // 픽스처가 없는 빌드. 가드를 빼면
            // 없는 타입을 참조해 빌드가 깨진다.
