...
- [ ] 손으로 작성한 레벨 JSON 최소 1개가 저장소에 커밋됨
- [ ] JSON에 공 시작점·목표 위치·잉크 제한이 포함됨
- [ ] 파일 → `LevelData` → 월드 구축이 테스트로 확인됨
- [ ] 역직렬화 값이 파일 내용과 일치함이 확인됨 (기본값으로 조용히 채워지지 않았다는 증명)
- [ ] 같은 배치의 C# 레벨과 시뮬 결과가 일치함
